          ...
          <section class="col-md-6 offset-md-3">
            ...
            <div class="panel" style="border:solid 3px #0A4833;background-color: white;">
              ...
              <div class="panel-body p-3">
                <form method="post" action="{{ route('histories.update',$editdata->id) }}" enctype="multipart/form-data" id="quickForm" >
                  @csrf
                <div class="form-row">
                  
                  <div class="form-group col-md-12">
                    <label for="title">Title</label>
                    <input type="text"class="form-control"  name="title" id="title" placeholder="Enter Title" value="{{ @$editdata->title }}">
                <font style="color:red">{{($errors)->has('title')?($errors->first('title')):''}}</font>
                  </div>
                   
                  <div class="form-group col-md-12">
                    <label for="details">Details</label>
                    <textarea class="form-control" name="details" id="details" placeholder="Enter Details" rows="5"> {{ @$editdata->details }}</textarea>
                      <font style="color:red">{{($errors)->has('details')?($errors->first('details')):''}}</font>
                  </div>

                    <div class="form-group col-md-12"> 
                    <img id="showimage" src="{{(!empty($editdata->image))?url('upload/historyimage/'.$editdata->image):url('upload/usernoimage.png')}}"
                   ...
                   <div class="form-group col-md-12">
                    <input type="submit" name="submit" value="Upadate History" class="btn btn-success">
